...
class foldergallery{
	...
	function fg_gallery( $atts ) { // Generate gallery
		...
		if ( !is_dir( $folder ) ) {
			return '<p style="color:red;"><strong>' . __( 'Erro:', 'foldergallery' ) . '</strong> ' .
				// sprintf( __( 'O diretório %s contendo as imagens não foi encontrado.', 'foldergallery' ), $folder ) . '</p>';
                                sprintf( __( 'O diretório contendo as imagens não foi encontrado.', 'foldergallery' ) ) . '</p>';	
		}
		...
		if ( 0 == $NoP ) {
			return '<p style="color:red;"><strong>' . __( 'Erro:', 'foldergallery' ) . '</strong> ' .
				// sprintf( __( 'No picture available inside %s.', 'foldergallery' ), $folder ) . '</p>';
                                sprintf( __( 'Nenhuma imagem encontrada no diretório.', 'foldergallery' ) ) . '</p>';
		}	
		// Cleanup parameters
		$width=intval($width);
		...
		for ( $idx = $start_idx ; $idx < $NoP ; $idx++ ) {
			...
			switch ( $fg_options['engine'] ) {
				...
				case 'photoswipe' :
				case 'none' :
					$gallery_code .= '<a class="colorbox_exp_image" title="' . $thecaption . '" href="' . home_url( '/' . $folder . '/' . $pictures[ $idx ] ) . '">';
				break;
			}
