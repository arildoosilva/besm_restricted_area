...
?>			
	...
	</div><div class="frontier_no_border">
	<fieldset class="frontier_post_fieldset">
		<legend><?php _e("Conteúdo", "frontier-post"); ?></legend>	
	...
	echo '<div class="frontier_no_border">';
	...
	if ( current_user_can( 'frontier_post_tags_edit' ) || fp_get_option_bool("fps_show_feat_img") )
		...
		if ( current_user_can( 'frontier_post_tags_edit' ) )
			...
			for ($i=0; $i<$fp_tag_count; $i++)
				...
				echo '<input placeholder="'.__("", "frontier-post").'" type="text" value="'.$tmp_tag.'" name="user_post_tag'.$i.'" id="user_post_tag"><br>';	
				}
			echo '</fieldset>';
			} 
	...
	?>
		...
		{ ?>
			<button class="button frontier_button" type="submit" name="user_post_submit" 	id="user_post_submit" 	value="savereturn">Publicar<?php //echo $frontier_return_text; ?></button>
		<?php }
		if ( fp_get_option_bool("fps_submit_cancel") )
		{ ?>
		<input type="reset" class="frontier_button" value="<?php _e("Cancelar", "frontier-post"); ?>"  name="cancel" id="frontier-post-cancel" onclick="location.href='<?php the_permalink();?>'">
		<?php } ?>
	</fieldset>
